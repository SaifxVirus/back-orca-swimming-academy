  ...
  return (
    <div className="app-shell" dir={language === 'en' ? 'ltr' : 'rtl'}>
      <aside className={`sidebar ${menuOpen ? 'open' : ''}`}>
        ...
        <div className="sidebar-bottom"><button type="button" onClick={() => { setView('الإعدادات'); setMenuOpen(false) }}><Settings size={18} /> الإعدادات</button><button type="button" className="logout-button" onClick={onLogout}>تسجيل الخروج</button><button type="button" className="user-card" onClick={() => { setProfileOpen(true); setMenuOpen(false) }}><div className="avatar">{userName.charAt(0).toUpperCase()}</div><div><b>{userName}</b><span>الملف الشخصي</span></div><ChevronLeft size={15} /></button></div>
      </aside>
      <main className="main-content">
        ...
        <div className="page-wrap">
          ...
          {view === 'الإعدادات' && <SettingsPage onProfileSaved={onProfileSaved} />}
          {profileOpen && <ProfileModal onClose={() => setProfileOpen(false)} onSaved={onProfileSaved} />}
...
function ProfileModal({ onClose, onSaved }: { onClose: () => void; onSaved: (name: string, language: 'ar' | 'en', token: string) => void }) {
  ...
  return <div className="modal-backdrop"><form className="modal profile-modal" onSubmit={save}><div className="modal-heading"><div><h2>الملف الشخصي</h2><p>عدّل بيانات الحساب واللغة المفضلة.</p></div><button type="button" className="modal-close" onClick={onClose}><X size={18} /></button></div><div className="form-grid"><label>الاسم<input required value={profile.name} onChange={(event) => setProfile({ ...profile, name: event.target.value })} /></label><label>البريد الإلكتروني<input required type="email" value={profile.email} onChange={(event) => setProfile({ ...profile, email: event.target.value })} /></label><label>كلمة مرور جديدة<input type="password" placeholder="اتركها فارغة بدون تغيير" value={profile.password} onChange={(event) => setProfile({ ...profile, password: event.target.value })} /></label><label>اللغة<select value={profile.language} onChange={(event) => setProfile({ ...profile, language: event.target.value as 'ar' | 'en' })}><option value="ar">العربية</option><option value="en">English</option></select></label></div><div className="modal-footer"><button type="button" className="secondary-button" onClick={onClose}>إلغاء</button><button className="primary-button" type="submit" disabled={saving}>{saving ? 'جار الحفظ...' : 'حفظ الملف الشخصي'}</button></div></form></div>
}

function SettingsPage({ onProfileSaved }: { onProfileSaved: (name: string, language: 'ar' | 'en', token: string) => void }) {
  const [form, setForm] = useState({ academyName: '', currency: 'جنيه مصري', alertDays: 7, allowNegativeStock: false, commissionRule: '' })
  const [profile, setProfile] = useState({ name: '', email: '', password: '', language: 'ar' as 'ar' | 'en' })
  const [saved, setSaved] = useState(false)
  useEffect(() => { api('/settings').then(setForm).catch(() => undefined); api('/profile').then((data) => setProfile({ name: data.name, email: data.email, password: '', language: data.language ?? 'ar' })).catch(() => undefined) }, [])
  ...
  const saveProfile = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); try { const result = await api('/profile', { method: 'PATCH', body: JSON.stringify(profile) }); localStorage.setItem('back_orca_token', result.token); onProfileSaved(result.user.name, result.user.language, result.token); window.alert('تم حفظ الملف الشخصي') } catch (error) { window.alert(error instanceof Error ? error.message : 'تعذر حفظ الملف الشخصي') } }
